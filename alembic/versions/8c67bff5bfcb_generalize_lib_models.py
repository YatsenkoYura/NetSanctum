...
down_revision: str | None = "4ee3ae65250e"
...
def upgrade() -> None:
    ...
    op.add_column(
        "lib_media", sa.Column("media_type", sa.String(length=50), nullable=False, server_default="novel")
    )
    ...
    op.create_foreign_key(
        "lib_chapters_media_id_fkey", "lib_chapters", "lib_media", ["media_id"], ["id"], ondelete="CASCADE"
    )
...
def downgrade() -> None:
    ...
    op.create_foreign_key(
        "ranobe_chapters_novel_id_fkey", "lib_chapters", "lib_media", ["media_id"], ["id"], ondelete="CASCADE"
    )
