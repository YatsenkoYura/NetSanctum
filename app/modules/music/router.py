...
from app.core.security import get_current_user, OwnerUser
from app.core.templates import templates
from app.core.storage import get_storage
import json
import redis
redis_client = redis.Redis(host='redis', port=6379, db=0, decode_responses=True)
from app.modules.music.models import Song, Playlist
...
async def api_list_playlist_songs(playlist_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    return [{
        ...
        "original_artist": s.original_artist, "youtube_url": s.youtube_url,
        "audio_url": f"/music/audio/{s.id}", "cover_url": f"/music/cover/{s.id}" if s.cover_file_id else None
    } for s in songs]


@router.get("/api/songs")
async def api_list_songs(search: str | None = None, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
...
async def api_download(req: DownloadRequest, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    if req.youtube_cookies:
        if req.youtube_cookies.strip().upper() == "CLEAR":
            ...
            if "# Netscape HTTP Cookie File" not in req.youtube_cookies and ".youtube.com" not in req.youtube_cookies:
            ...
            await settings_service.upsert_setting(
                db, key="youtube_cookies", value=req.youtube_cookies, scope="global", value_type="string", is_secret=True
            )
    task = process_youtube_url_task.delay(req.url, req.use_ai, req.openai_api_key, req.openai_base_url, req.playlist_id)
    data = {
        "task_id": task.id,
        "url": req.url,
        "title": "Resolving URL...",
...
@router.delete("/ui/playlists/{playlist_id}/songs/{song_id}", response_class=HTMLResponse, include_in_schema=False)
...
async def music_playlist_detail_ui(
    ...
    return templates.TemplateResponse(
        ...
        {"playlist": playlist, "songs": songs, "lang": lang}
    )
...
async def get_playlist_library_ui(
    ...
    from sqlalchemy import select
    from app.modules.music.models import Song, PlaylistSong
    ...
    for song in songs:
        ...
        btn = f'<span class="text-[10px] font-mono text-zinc-500 bg-zinc-900 border border-zinc-800 px-2 py-0.5">{_t("in_list", lang)}</span>' if is_added else f"""
        ...
        if cover_url:
            cover_html = f'<img src="{cover_url}" class="w-6 h-6 object-cover rounded-sm border border-zinc-800">'
        else:
            cover_html = """
...
@router.post("/ui/playlists/{playlist_id}/songs/{song_id}", response_class=HTMLResponse, include_in_schema=False)
async def add_song_to_playlist_ui(
    ...
    exists_stmt = select(PlaylistSong).where(PlaylistSong.playlist_id == playlist_id, PlaylistSong.song_id == song_id)
    ...
    pos_stmt = select(func.coalesce(func.max(PlaylistSong.position), -1)).where(PlaylistSong.playlist_id == playlist_id)
    ...
    response = HTMLResponse(f'<span class="text-emerald-400 font-bold font-mono text-[10px]">{_t("added", lang)}</span>')
...
async def start_download(
    ...
    if not url:
        return HTMLResponse('<div class="text-red-500 font-mono text-xs border border-red-500 p-2">URL required.</div>')
    ...
    try:
        ...
        return HTMLResponse(f'<div class="text-red-500 font-mono text-xs border border-red-500 p-2">{e.detail}</div>')
